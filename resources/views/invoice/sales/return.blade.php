...
@section('title',__('مرتجعات المبيعات'))

@section('content')

    <div class="panel">
        <div class="panel-heading">
            <h3 class="panel-title">{{__('مرتجعات المبيعات')}}</h3>
        </div>
        <!--Data Table-->
        <!--===================================================-->
        <div class="panel-body">
            <div class="row">
                <form  method="post" action="{{route('invoice.sales.returned.filter')}}">
                    @csrf
            ...
            <table  id="table" class="table ">
                ...
                @foreach($returned as $return)
                    <tr>
                        ...
                        <td>{{ $return->invoice->customer->name }}</td>
                        <td>{{ $return->invoice->note }}</td>

                        @canany(['update invoice', 'read invoice'])
                            <td>
                                @can('read invoice')
                                    <a href="{{ route('invoice.sales.printReturn', $return) }}"  class="btn btn-success fa fa-eye"></a>
